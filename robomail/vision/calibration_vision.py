...
class CalibrationClass():
    ...
    def prepare_dataset(self, voxel_size, source, target, transform=np.identity(4)):
        """
        1. Accepts voxel size and 2 loaded pointclouds: the source and target respectively
        2. Reads input .ply files and defines source and target point clouds
        3. Removes statistical outliers and crops incoming pointclouds using remove_outliers and remove_background 
        4. Visualizes raw source and target point clouds in initial configuration (transform = np.identity(4))
        5. Downsamples point clouds and calculates FPFH's using preprocess_point_cloud
        6. Returns source, target, downsampled source and target, and source and target FPFH's 
        """
        
        print("\nDisplaying raw alignment...")
        self.draw_registration_result(source, target, transform)

        source_down, source_fpfh = self.preprocess_point_cloud(source, voxel_size * 3) # compute FPFH and downsampled point cloud for source and target 
    ...
    def preprocess_point_cloud(self, pcd, voxel_size):
        ...
        2. Estimate normals
        3. Compute a FPFH feature (a 33-dimensional vector that describes the local geometric property of a point) for each point 
        """
        print("\nDownsampling with a voxel size %.3f..." % voxel_size)
        pcd_down = pcd.voxel_down_sample(voxel_size * 0.5) # global registration is only performed on a heavily down-sampled point clouds
    ...
    def execute_global_registration(self, source_down, target_down, source_fpfh, target_fpfh, voxel_size):
        """
        Applies RANSAC global registration method 
        """
        ...
        result = o3d.pipelines.registration.registration_ransac_based_on_feature_matching(
            ...
            criteria = o3d.pipelines.registration.RANSACConvergenceCriteria(max_iteration = 4000000, confidence = 1.0))
    ...
    def refine_registration(self, source, target, voxel_size, trans_global):
        """
        Refine global registraction with local refinement (ICP) 
        """
        ...
        evaluation = o3d.pipelines.registration.evaluate_registration(source, target, distance_threshold, trans_global)
        print(evaluation)
